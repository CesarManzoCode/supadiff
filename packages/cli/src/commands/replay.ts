 ...
 * carry no live secrets to begin with, so replay always supplies fresh local-safe
 * defaults for them rather than expecting the artifact to carry them.
 */
function recipeToTargetSpec(recipe: SanitizedRecipeJson): TargetSpec {
  return {
...
function candidateRecipes(files: Map<string, Buffer>): TargetSpec[] {
